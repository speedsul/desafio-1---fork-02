...
function addToBuffer() {
  ...
  filteredList = resultList.filter((person) => {
    wfilter = new RegExp(keyboardBuffer, 'i');
  ...
  waverageAge.innerText = (
    totals.totalAge / (totals.countFemale + totals.countMale) || 0
  ).toFixed(2);
}

async function start() {
  // needed to make it work without "CORS" blocking
  ...
  resultList = fullResponseList.results.map((person) => {
